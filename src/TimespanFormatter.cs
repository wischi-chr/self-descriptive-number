    ...
    public class HMSFormatter : ICustomFormatter, IFormatProvider
    {
        ...
        public string Format(string format, object arg, IFormatProvider formatProvider)
        {
            if (arg != null)
            {
                ...
                switch (format)
                {
                    case "S": fmt = String.Format(new HMSFormatter("{0} Seconds", "{0} Second"), "{0}", arg); break;
                    case "M": fmt = String.Format(new HMSFormatter("{0} Minutes", "{0} Minute"), "{0}", arg); break;
                    case "H": fmt = String.Format(new HMSFormatter("{0} Hours", "{0} Hour"), "{0}", arg); break;
                    case "D": fmt = String.Format(new HMSFormatter("{0} Days", "{0} Day"), "{0}", arg); break;
                    default: fmt = String.Format((int)arg > 1 ? _plural : _singular, arg); break;
                }
                return fmt;
            }
